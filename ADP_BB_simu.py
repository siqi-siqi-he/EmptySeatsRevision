...
def E(j,c):
    #define a vector where j component is 1, otherwise 0
...
def findseats(rand,p1,pj):
    #given a random number, choose which seat i is purchased
    sum=p1
...
def Simulation(random):
    ...
    #for different selling horizons:
    for t in range(T,0,-1):
        ...
        #calculate the prices based on the results
        ...
        #given the random number, find out which product is bought
        if random[t - 1] > p10 + p20 + p30:
            ...
            #buy product 2 (find which seat)
            seat = findseats(random[t - 1], p10, p2j0)
            if x[seat] == 0:
                #this situation shouldn't happen, but used for debugging
                print("seat is purchased beforehand")
                continue
            ...
            #buy product 3, find out which seat
...
for j in range(1,5):
    #for j in range(1, K): meaning for c taking from 8 to 8K-8
    c=j*8
    T=c*2
