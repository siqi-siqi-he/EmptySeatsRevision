...
def pricing_aisle_window_seats(c):
    a0 = 0
    a1 = 0.2
    w = np.sort(np.concatenate((np.arange(0, c, 4), np.arange(3, c, 4))))
    ai = np.sort(np.concatenate((np.arange(1, c, 4), np.arange(2, c, 4))))
    a2 = 0.4 * np.ones(c)
    a2[w] = 0.5
    a3 = 0.6 * np.ones(c)
    b1 = 0.2
    ...
    tau2 = 0.4
    tau3 = 0.6
    return a0, a1, a2, a3, b1, b2, b3, tau0, tau1, tau2, tau3, w, ai

def pricing_increasing_seats(c):
    add = np.linspace(0, 0.01 * c - 0.01, c)
    addhelp = (add[1::2] + add[0::2]) / 2
    add2 = np.tile(addhelp, 2).reshape(2, -1).T.flatten()
    a0 = 0
    a1 = 0.2
    a2 = 0.4 * np.ones(c) + add
    a3 = 0.6 * np.ones(c) + add2
    b1 = 0.2
    b2 = 0.4
    b3 = 0.6
    tau0 = 1
...
def par_func(ran_seed,attempt, success, x1, Y, m, c, coeffs, allstates, t, a0, a1, a2, a3, b1, b2, b3, tau1, tau2, tau3, eps, mode, VFA, w, ai):
    t1,t2 = m
    vmt = np.zeros(t2-t1)
    for mcur in range(t1,t2):        
        x = x1[:, mcur]
        y = Y[mcur]
        try:
            ofv = opt_price_BADP_coeff(c, x, y, coeffs, allstates[mcur, :], t, a0, a1, a2, a3, b1, b2, b3, tau1,
            ...
            attempt+=1
        vmt[mcur]= ofv
    return [vmt,success,ran_seed,attempt]

if __name__ == '__main__':
    eps = 0
    num_processes = 8 # Adjust this to the number of cores available
    choice = 2      #0=homogeneous, 1=aisle/window, 2=heterogeneous
    VFA = 4         #note that VFA 0,1,2 refer to the choice 0,1,2 respectively, other VFA might be used for all choices 
    mode = 'cvx'    #alternatives: 'cvx' and 'scipy'
    for choice in [0]:
        for c in range(8,9,8):
            approx = 0
            T = 2 * c
            ...
            if choice == 0:
                ...
                a0, a1, a2, a3, b1, b2, b3, tau0, tau1, tau2, tau3, w, ai = pricing_aisle_window_seats(c)
            elif choice == 2:
                a0, a1, a2, a3, b1, b2, b3, tau0, tau1, tau2, tau3 = pricing_homo_seats(c)
                w = 0 
                ai = 0
            ...
            for M in [16]:
                ...
                for t in range(1, T + 1):
                    ...
                    while success==False:
                        ...
                        args = [(ran_seed,attempt, success, x1, Y, m, c, coeffs, allstates, t, a0, a1, a2, a3, b1, b2, b3, tau1, tau2, tau3, eps, mode, VFA, w, ai) for t_period in ranges]
                        # Use a Pool to parallelize the computation
            
                        with Pool(processes=num_processes) as pool:
                            results = pool.starmap(par_func, args)
            
                        for process in range(1,num_processes):
                            vmt = np.concatenate((vmt,results[process][0]),axis=0)
