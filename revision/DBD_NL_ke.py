...
def r2(j, p1, p2, p3):
    #price of product 2 given probability p1 p2 p3
    if p2[j] <= 0:
        result = 0
    elif sum(p2) <= 0:
...
def r3(j, p1, p2, p3):
    #price of product 3 given probability p1 p2 p3
    if p3[j] <= 0:
        result = 0
    elif sum(p3) <= 0:
...
def computeDP(choice):
    ...
    start=time.time()
    for i in range(c):
        #for different component i in the approximation function
        start_i = time.time()
        V = np.full((T + 1, 2), 0.0)
        ...
        #we find the upper bound among all the components in x, however, to find the final upper bound of DBD_ke, we also
        #need to compare it with the y from DBD
