...
class BBNode():
    ...
    def can_be_integral(self, bool_vars_r, p0,p1, p2j, p3j,y):
        value = self.func(p0,p1, p2j, p3j, bool_vars_r,y)
        return value
    ...
    def branch(self):
        ...
        for branch in [0, 1]:
            ...
            ymin = (abs(self.y_vars.value - 1.5))
            if xmin < ymin:
    ...
    def bbsolve(self):
        ...
        while len(heap) > 0:
            ...
            if y_vars_r==0:
                p3j_vars_r = [0 for i in range(len(p3j_vars_r))]
            p1_vars_r = np.squeeze([v.value for v in node.p1_vars])
            p0_vars_r = np.squeeze([v.value for v in node.p0_vars])

            res_r = node.can_be_integral(bool_vars_r=bool_vars_r, p0=p0_vars_r,p1=p1_vars_r, p2j=p2j_vars_r, p3j=p3j_vars_r,y=y_vars_r)
